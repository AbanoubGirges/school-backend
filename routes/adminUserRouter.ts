...
import getResetPasswordRequestsController from "../Controllers/adminControllers/onUser/getResetPasswordController.js";
...
adminUserRouter.get('/reset-password',authAdmin,getResetPasswordRequestsController);
adminUserRouter.put('/reset-password',authAdmin,[body('newPassword').isLength({min:6}).not().isEmpty().withMessage('new password is required'),body('requestId').notEmpty().withMessage('request ID is required')],);
